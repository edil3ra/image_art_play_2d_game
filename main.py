...
def generate(name):
    ...
    data = yaml.full_load(
        open(os.path.join(default_data_directory, f"{name}.yaml")).read()
    )
    generate_assets(Config.parse_obj(data))
...
def list_directory(directory):
    ...
    paths = [
        "{} - {}".format(basename, fullname)
        for (basename, fullname) in zip(basenames, fullnames)
    ]
    click.echo("\n".join(paths))
